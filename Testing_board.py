...
class Board:
    ...
    def update(self):
        ...
        if pyxel.btnp(pyxel.KEY_Q):
            ...
            if self.mario.direction != None:
                self.mario.update_position()
    ...
    def update_mario_direction(self):
        ...
        if pyxel.btn(pyxel.KEY_RIGHT):
            ...
            self.mario.direction = 'up'
        elif pyxel.btn(pyxel.KEY_DOWN):
            self.mario.direction = 'down'
        else:
            self.mario.direction = None
    ...
    def check_death(self):
        # We compare x and y position of Mario and each barrel object
        # to check if they have collided
        for barrel in self.barrels:
            if (abs(self.mario.x - barrel.x) <= 8) \
            and (abs(self.mario.y - barrel.y) <= 8) and self.mario.is_alive:
                self.mario.is_alive = False
                self.mario.death_event()
    
    def gain_score(self):
        # This function implements the score system of the game:
        # If Mario jumps over a barrel gets 100 points
        for barrel in self.barrels:
